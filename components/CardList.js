...
class CardList extends React.Component {
  render() {
    const queueCardListNode = this.props.queueData.map((dataItem) => {
      return (
        <CardItem title={dataItem.title} priority={dataItem.priority} status={dataItem.status} assignedTo={dataItem.assignedTo} createdBy={dataItem.createdBy} id={dataItem.id} key={dataItem.id}/>
      )
    })
